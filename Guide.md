Here's a comprehensive Python program that can convert images to PDF files:
...
|----------|-------------|
| `convert_single_image_to_pdf()` | Converts one image to one PDF |
| `convert_multiple_images_to_one_pdf()` | Merges multiple images into one PDF |
| `convert_multiple_images_to_multiple_pdfs()` | Converts each image to its own PDF |
| `get_images_from_directory()` | Scans a folder for image files |

### Menu Options
1. **Single image → PDF** - with optional custom output path
2. **Multiple images → One PDF** - merges all into a single file
3. **Multiple images → Multiple PDFs** - each image gets its own PDF
4. **Folder images → One PDF** - auto-detects all images in a folder
5. **Folder images → Multiple PDFs** - converts all folder images separately
...
### Key Behaviors
- **Transparency handling** - RGBA/PNG images get a white background before conversion
- **Validation** - checks file existence, extension, and image integrity
- **Auto output paths** - generates output paths if not specified
- **Partial success** - skips invalid files and continues with valid ones
- **Conversion summary** - shows success/fail counts after batch operations
